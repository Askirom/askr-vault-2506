## 2025-09-30 Abstimmung KI
- OSS Modell
	- mit und ohne Internet
- DA liegt als Entwurf vor
- https://www.stmfh.bayern.de/digitalisierung/ki/
- https://www.bmas.de/DE/Service/Publikationen/Broschueren/a862-leitlinien-ki-einsatz-behoerdliche-praxis-arbeits-sozialverwaltung.html

# 2025-11-24 DSI Webseiten

**Teilnehmer:** 
- Robin Leitner (DSB)
- Marina Stoll (Leitung Stabsstelle)
- Heinz-M. Gräsing (IT/Verwaltung - *Anwesenheit vermutet, da Termin wegen seines Urlaubs verschoben wurde*)
## Ziele des Termins
1. **Statusaufnahme Webseiten:** Klärung der eingebundenen Dienste für die Aktualisierung der Datenschutzerklärungen (DSE).
2. **Social Media Compliance:** Beratung zum Einsatz von Facebook (und anderen Plattformen) gemäß Vorgaben und aktueller Rechtsprechung.

---

## Agenda & Besprechungspunkte
### 1. Webseiten (DSE Aktualisierung)
*Hintergrund: DSEs sind nicht mehr aktuell (Mail Gräsing 31.07.).*

**Aufgabe:** Klären, welche externen Dienste ("Dienste") auf den folgenden Domains laufen (z.B. Matomo, Maps, Fonts, Videos), um die DSE anzupassen.

**Liste der Webseiten (von Fr. Stoll):**
- [ ] treuchtlingen.de
	- Matomo
		- liest Apachelogs aus zur Auswertung
		- Anonymisierung wird durch Matomo übernommen
- [ ] umweltstation.treuchltingen.de
	- reine Infoseite 
	- Typo3 Seite
- [ ] museum.treuchtlingen.de
	- Typo3 Seite
	- reine Infoseite
- [ ] mitgestalten.treuchtlingen.de
	- Typo3 Seite
	- reine Infoseite
	- mit Links auf andere Seite
- [ ] grundschule.treuchtlingen.de
	- Typo3 Seite
	- reine Infoseite
	- 
- [ ] treuchtlingen.vision
- [ ] digitalisierung.treuchtlingen.de
- [ ] info.treuchtlingen.de
- [ ] feuerwehr.treuchtlingen.de
- [ ] tourismus-treuchtlingen.de
	- Matomo
	- Mit Alexandra Lindert muss die Tourismus-Seite bewertet werden
	- dort wird TIM4 genutzt
	- darin auch shop.treuchtlingen.de
		- Cookie-Banner fehlt ablehnen
- [ ] umfragen.treuchtlingen.de
	- alle Umfragen gehen auf die Domain beziehungsweise spezifische Formularseite
	- LimeSurvey
- [ ] wann.treuchtlingen.de
	- DSI fehlt
- Bürger Service Portal
	- AKDB
	- Bindung ans BayernPortal
	- keine Alternative für Treuchtlingen
	- DSI Verlinkung passt nicht
	- DSI Teil fehlt
- 


### 2. Social Media & Plattformen
*Hintergrund: Beratungswunsch bzgl. Facebook Pflichtpunkte & Urteil Köln (Mail Gräsing 31.07.).*

**Genutzte Plattformen:**
- Facebook (stadt.treuchtlingen)
- Instagram (treuchtlingen.laedtdichauf)
- YouTube (@stadttreuchtlingen518)
- Pinterest (treuchtlingenlaedtdichauf)

**To-Discuss (Fokus Facebook):**
- [ ] **Art. 13 & 14 DSGVO:** Bereitstellung einer detaillierten DSE (Gemeinsame Verantwortlichkeit & Drittlandtransfers).
- [ ] **Art. 26 Vereinbarung:** Umgang mit Metas Standard-Addendum (Dokumentation der Unzulänglichkeit).
- [ ] **Rechtliche Einordnung:**
    - Urteil Köln / Bundesebene (Fanpages).
    - Empfehlung des Bayerischen Landesbeauftragten für den Datenschutz (BayLfD).
- [ ] **Prozess:** Wie soll die regelmäßige Überprüfung der Compliance-Bemühungen dokumentiert werden?

---

## Action Items / To-Dos (nach Meeting)
- [ ] Protokoll versenden
- [ ] Entwürfe für DSE der Webseiten erstellen
- [ ] Stellungnahme/Handlungsempfehlung Facebook finalisieren

---

## Notizen
*Hier während des Meetings mitschreiben...*

- Cookiebanner versperrt Datenschutzerklärung
- Cookies
	- es gibt keine
- Richtlinie
	- keine iframes etc.
	- nur externe Links
- formulare.treuchtlingen.de
- IT klärt wo Cookies gespeichert werden 
	- für alle Webseiten
- MuT App
- Heinz fragt bei Fürth nach
- 